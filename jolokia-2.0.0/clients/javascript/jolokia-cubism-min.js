"use strict";(function(root,factory){if(typeof define==="function"&&define.amd){define(["cubism","./jolokia"],factory);}else if(typeof module==="object"&&module.exports){module.exports=factory(require("cubism"),require("./jolokia"));}else{if(root.Jolokia){factory(root.cubism,root.Jolokia);}else{console.error("No "+(root.cubism?"Cubism":"Jolokia")+" definition found. "+"Please include jolokia.js and cubism.js before jolokia-cubism.js");}}}(typeof self!=="undefined"?self:this,function(cubism,Jolokia){var builder=function(cubism,Jolokia){var VERSION="2.0.0";var ctx_jolokia=function(url,opts){var source={},context=this,j4p=createAgent(url,opts),step=5e3;try
...
values.length=i;}}
callback(null,retVals);}}};ctx_jolokia.VERSION=VERSION;cubism.context.prototype.jolokia=ctx_jolokia;return ctx_jolokia;};return builder(cubism,Jolokia);}));